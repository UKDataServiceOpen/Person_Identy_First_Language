...
pdffileobj = open('processed_dataset/ESHG2010Abstracts.pdf', 'rb') 
# -
...
for i in range(x):
   ...
   # Append-adds at last
   file1 = open("processed_dataset_text/ESHG2010Abstracts.txt", "a", encoding='utf-8')  # append mode
   file1.write(text)
   file1.close()
...
file1 = open("processed_dataset_text/ESHG2010Abstracts.txt", "r", encoding='latin-1')
...
listt = re.split(r'(\s*\b[A-Z]+\d+\.\d+)|(\n\b[a-z]+\d+\.\d+)|(\b[A-Z]+\d+\.\d+\*)', whole_file)
# -
...
df2['Session_Code'] = df['Title'].str.findall(('^[A-Z]+\d+\.\d+\*') or ('^[A-Z]+\d+\.\d+'))

# separate out session code from title using RE
# Keep session code upto 3 digits after .
df2['Session_Code'] = df['Title'].str.findall( ('^[A-Z]+\d+\.\d{1,3}'))
# -
