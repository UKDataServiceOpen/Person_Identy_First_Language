...
pdffileobj = open('processed_dataset/ESHG2008Abstracts.pdf', 'rb') 
# -
...
for i in range(x):
   ...
   # Append-adds at last
   file1 = open("processed_dataset_text/ESHG2008Abstracts.txt", "a", encoding='utf-8')  # append mode
   file1.write(text)
   file1.close()
...
file1 = open("processed_dataset_text/ESHG2008Abstracts.txt", "r", encoding='latin-1')
...
listt = re.split(r'(\s*\b[A-Z]+\d+\.\d+)', whole_file)
# -
...
for i in range(len(new)):
    if re.search("^[A-Z]+\d+\.\d+\s*", new[i]) is not None:
        print(new[i])
        a=i
        a+=1
        new[a]+=' '
...
df3.to_csv('results/ESHG2008Abstracts.csv', index = False)  
